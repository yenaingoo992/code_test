...
class UpcomingMovieDataSource @Inject constructor(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Movie> {
        ...
        return try {
            val response = api.getUpcomingMovies(page = position)
            val data = response.body()
            if (response.isSuccessful && data != null) {
                if (position <= data.totalPages) {
                    position += 1
                }
                dao.insertAll(data.toEntity("upcoming"))
                val favIds = dao.getFavMovieIds()
                LoadResult.Page(data = data.toModel("upcoming").map {
            ...
            val cachedMovies = dao.getUpcomingMovies()
